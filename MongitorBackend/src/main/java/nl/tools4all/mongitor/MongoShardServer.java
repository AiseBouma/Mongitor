...
public class MongoShardServer extends MongoServer
{
   ...
   * @param state
   */
  public MongoShardServer(String id, String hostname, int port)
  {
  ...
  public String check()
  {
            ...
            } else
            {
              ...
              else
              {
                ...
                {    
                  ...
                  {
                    MongoShardServer member = ReverseServerIndex.getShardServer(name);
                    if (member == null)
                    {
    ...
    catch (Exception e)
    {
      ...
      int errmsgpos = exceptionmessage.indexOf("\"errmsg\" : \"");
      if (errmsgpos > -1)
      {
        int errmsgend = exceptionmessage.indexOf("\"", errmsgpos+12);
        if (errmsgend > -1)
        {
          result = result + ": " + exceptionmessage.substring(errmsgpos+12, errmsgend);
        }
      }
      mongitorstate = 3;
    }
